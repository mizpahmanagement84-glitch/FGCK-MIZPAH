...
function NavBar({ user, onLogout }) {
  ...
  const displayRole = role === 'pastor' ? 'Pastor' : role === 'secretary' ? 'Secretary' : role === 'member' ? 'Member' : 'Admin';
  const welcomeName = role === 'pastor' ? 'Pastor Robert' : role === 'secretary' ? 'Secretary' : user?.username || 'Member';
  return (
    <aside className="navbar">
      <div>
        <h2>Mizpah {displayRole}</h2>
      ...
      <nav className="nav-links">
        ...
        {role === 'pastor' && <NavLink className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'} to="/givings">Offerings</NavLink>}
        {role === 'pastor' && <NavLink className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'} to="/projects">Project</NavLink>}
        {role === 'pastor' && <NavLink className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'} to="/welfare">Welfare</NavLink>}
        {role === 'pastor' && <NavLink className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'} to="/reports">Tithe</NavLink>}
        {role === 'pastor' && <NavLink className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'} to="/expenses">Expenses</NavLink>}
        {role === 'pastor' && <NavLink className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'} to="/attendance">Attendance</NavLink>}
        {role === 'pastor' && <NavLink className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'} to="/inventory">Inventory</NavLink>}
        {role === 'pastor' && <NavLink className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'} to="/departments">Departments</NavLink>}
