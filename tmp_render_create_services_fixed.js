...
(async () => {
  try {
    ...
    if (backendId) {
      ...
      await createEnvVar(backendId, { key: 'DATABASE_URL', value: 'postgresql://<username>:<password>@<region>.neon.tech:5432/<database>?sslmode=require' });
    }

    console.log('Render services created successfully.');
    console.log('Backend service ID:', backendId);
    console.log('Client service ID:', client.body?.id);
    console.log('Neon connection string format: postgresql://<username>:<password>@<region>.neon.tech:5432/<database>?sslmode=require');
    console.log('Replace DATABASE_URL in mizpah-server env vars with your actual Neon connection string.');
  } catch (error) {
